...
### ✅ Artes de Divulgação — implementado (5 tipos)
...
- **Overlay + scrim**: overlay base leve (0.25) na imagem toda + scrim em gradiente vertical (transparente no topo → escuro na região do texto, aproximado por faixas horizontais no GD) para contraste garantido em qualquer foto
- **Logo**: sobreposto no canto superior esquerdo quando `logo` está definido (com suporte a transparência PNG); os templates de faixa clampam o topo da faixa abaixo da área do logo (`PADDING + 160`) para nunca cobri-lo no formato post
- **Tipografia**: instâncias **estáticas** da Lexend (`resources/fonts/Lexend-{Regular,Medium,SemiBold,Bold,Black}.ttf`, licença OFL) — a Lexend variável renderiza só a instância padrão no GD (sem bold real), então cada peso é um TTF próprio. Escala: título-herói 84–120px Black, secundário 44–56px SemiBold, kicker 30px SemiBold na cor de acento, meta 30–34px
- **Texto**: kicker + barra de acento + nome do evento (título-herói, quebra automática) + data (linha própria, SemiBold) + local resumido + CTA em pílula centralizada. Sem parágrafos longos: a descrição/tagline não entra na arte (vai na legenda da publicação). `EventMeta` (`App\Services\SocialAssets\EventMeta`) formata data pt-BR e resume o local ("Uni 7 - Av. ... - Fortaleza - CE" → "Uni 7 · Fortaleza - CE", com fallback seguro)
- **Rodapé de patrocínio**: se o evento tem patrocinadores, todas as artes ganham rodapé com kicker "PATROCÍNIO" + logos em chips brancos arredondados (fileira centralizada, altura uniforme via `contain()`, até 6 com escala decrescente, fallback de nome em texto quando não há logo). Implementado uma vez em `SocialAssetCanvas::drawSponsorFooter()`, chamado pelo service após o `compose()` do template — nenhum template precisa saber do rodapé. No story, a fileira fica na borda da safe zone do Instagram (~250px do rodapé); os templates reservam espaço para o CTA não colidir
...
- **Arquitetura multi-template** (`.claude/plans/majestic-greeting-donut.md`): os primitivos de desenho (fundo, overlay, logo, bloco de texto medido) vivem em `App\Services\SocialAssets\SocialAssetCanvas`, desacoplados de `Event`. Cada tipo de arte é uma classe pequena em `App\Services\SocialAssets\Templates\` implementando `SocialAssetTemplate::compose()`. `EventSocialAssetService` é um orquestrador: monta fundo/overlay/logo, resolve o template pelo `type` e delega o conteúdo.
  ...
  - `SponsorSpotlightTemplate` (Fase 3) — divulga um patrocinador específico: logo em cartão branco usando `contain()` (preserva proporção, sem cortar — essencial pra logo com fundo transparente), ou nome do patrocinador como fallback de texto quando não há `logo_url`; selo com o nível traduzido (`rapadura_com_castanha` → "Rapadura com Castanha" etc.), nome, evento+data, CTA "Conheça nosso patrocinador". Exige `sponsor_id`; `EventSocialAssetController` valida que o patrocinador pertence ao evento (404).
  - `SellingOutTemplate` (Fase 4) — urgência fixa, sem assunto: faixa vermelha (`--color-danger` do design system, `#dc2626`) com "INGRESSOS ESGOTANDO", chamada "Garanta o seu agora antes que acabe!", nome+data+local do evento, CTA "Garanta já" na mesma cor de alerta. Mensagem sempre fixa (não depende de `max_attendees`/participantes importados nem de `ticket_url` configurado) — decisão tomada com o organizador para não quebrar quando esses dados não estiverem em dia.
  - `TomorrowTemplate` (Fase 5) — lembrete sem assunto: faixa na cor secundária do site com "É AMANHÃ!", "Prepare-se, a gente se vê lá!", nome+data/hora (`\à\s H\hi`, ex. "às 19h00")+local, CTA "Não perca!". Gera a qualquer momento, sem bloquear pela proximidade real de `starts_at` — decisão tomada com o organizador (útil pra preparar a arte com antecedência).
...
- **Origem do avatar/logo**: mesma resolução R2-primeiro-depois-HTTP do `SocialAssetCanvas::fetchImage()` usada para capa/logo do evento — reaproveitada, não duplicada, tanto pelo palestrante quanto pelo patrocinador. O fetch remoto tem **retry (3× com 250ms)** e **cache de bytes por instância** (inclusive de falha): sem isso, logos de patrocinadores em hosts externos intermitentes apareciam numas artes e caíam no fallback de texto noutras, dentro da mesma rodada de geração.
- **Primitivos de design no `SocialAssetCanvas`**: `drawScrim` (gradiente por faixas), `drawAccentBar`, `drawButton` (pílula com texto centrado de verdade via `capHeight` — nunca chutar offset), `drawPillLabel` (badge), `drawRoundedCard` (3 retângulos + 4 círculos, GD não tem retângulo arredondado; com sombra sutil), `drawCircularAvatar` (recorte circular per-pixel via `core()->native()` + `imagesetpixel`, a lib não tem máscara circular; anel de acento por trás), `measureTextBlock` (mede sem desenhar, para layouts ancorados).
- **Download real**: `GET /admin/api/events/{event}/social-assets/{asset}/download` baixa o PNG do R2 no servidor (sem CORS, é server-to-server) e responde via `Storage::download()` com `Content-Disposition: attachment`. O botão "Baixar PNG" aponta pra essa rota — usar `asset_url` (R2, outra origem) direto no `<a download>` não funciona porque navegadores ignoram o atributo `download` em links cross-origin.
- Plano completo (5 fases, todas implementadas) em `.claude/plans/majestic-greeting-donut.md`.
- 31 testes de feature no total: 11 do `AnnouncementTemplate` + download (story, post, formato inválido, tipo inválido, evento inexistente, fallback sem capa/logo, nome com caracteres especiais, regenerar não duplica registro, `show` retorna artes já geradas, download força attachment, download de outro evento 404) + 6 do `SpeakerSpotlightTemplate` (gerar com sucesso, `talk_id` obrigatório, talk não aprovada rejeitada, talk de outro evento rejeitada, fallback sem avatar, regenerar não duplica) + 5 do `SponsorSpotlightTemplate` (gerar com sucesso, `sponsor_id` obrigatório, patrocinador de outro evento rejeitado, fallback sem logo, regenerar não duplica) + 3 do `SellingOutTemplate` (gerar story, gerar post sem `ticket_url`, regenerar não duplica) + 4 do `TomorrowTemplate` (gerar story, gerar post, gerar mesmo sem ser amanhã de fato, regenerar não duplica) + 2 do rodapé de patrocínio (gera com patrocinadores, gera sem patrocinadores)
