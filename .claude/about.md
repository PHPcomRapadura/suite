...
- **Frontend**: `EventSocialAssets.vue` com seletor de formato, preview e download; ao carregar a tela, mostra a arte já gerada (com data de geração) sem precisar clicar em "Gerar" de novo; card "Artes de Divulgação" no hub do evento
- **Arquitetura multi-template** (Fase 1 de `.claude/plans/majestic-greeting-donut.md`): os primitivos de desenho (fundo, overlay, logo, bloco de texto medido) vivem em `App\Services\SocialAssets\SocialAssetCanvas`, desacoplados de `Event`. Cada tipo de arte é uma classe pequena em `App\Services\SocialAssets\Templates\` implementando `SocialAssetTemplate::compose()`; hoje só existe `AnnouncementTemplate` (a chamada genérica do evento). `EventSocialAssetService` virou um orquestrador: monta fundo/overlay/logo, resolve o template pelo `type` e delega o conteúdo. Isso prepara o terreno para os próximos tipos (palestrante, patrocinador, ingressos esgotando, é amanhã) sem duplicar a lógica de canvas/medição de texto.
- 9 testes de feature (story, post, formato inválido, tipo inválido, evento inexistente, fallback sem capa/logo, nome com caracteres especiais, regenerar não duplica registro, `show` retorna artes já geradas)
